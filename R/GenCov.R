...
#' (i.e., not a correlation matrix by itself).
#'
#' *Copyright notice*: The "MAP" and "Givens" algorithms originally derived
#' from \code{rMAP()} and \code{rGivens()}, respectively, of the package
#' \code{fungible} version 1.99 (Waller, 2021), which is under GPL (>= 2).
#' (Strictly speaking, these were first adopted from the supplemental material
#' of Waller \[2020\], which is licensed under CC-BY-4.0.)
#' The adopted blocks are indicated in the source code.
#' Nevertheless, the coding here has modestly diverged from the original version
#' after modifications.
#'
#' @param p
#'   Number of variables.
#' @param VR
#'   The relative eigenvalue variance of the generated cov matrix;
...
#' @param shape
#'   Option to specify the "shape" of cov matrix;
...
#'     \item{elongate}{Special case of q-large with \code{q = 1}}
#'     \item{linearly_decreasing}{Eigenvalues linearly decreasing in magnitude}
#'     \item{quadratically_decreasing}{Eigenvalues quadratically decreasing}
#'   }
#'   User-specified VR is ignored with the last two options.
...
#'       order \eqn{p}.}
#'     \item{random}{Random eigenvectors are generated by QR decomposition
...
#'       (scaled so that the trace equals p); based on the iterative
#'       algorithm of Waller (2020). Modified from \code{fungible::rMAP()}.}
#'     \item{Givens}{Similar to MAP, but with the algorithm of
#'       Davies & Higham (2000) (based on \code{fungible::rGivens()});
#'       this is much faster and is guaranteed to converge.}
#'   }
#'   Alternatively, matrix of eigenvectors can be provided to specify
#'   full or partial eigenvectors for the outcome cov matrix.
...
#' GenCov(4, 0.5 ^ 2, evectors = "Givens", random_rotation = FALSE)
#'                               # Optionally with which_rotate = "head"/"tail"
#' GenCov(4, 0.5 ^ 2, evectors = "Givens", seed = 1234)
...
GenCov <- function(p = length(evalues), VR = 0.5, scale. = NULL,
    ...
    if(evectors[1] == "Givens") {
        ##########
        ## Start of adopted block 1: largely adopted from
        ## fungible::rGivens with modifications
        if(random_rotation) {
        ...
        while(length(y) > 0 && length(z) > 0) {
            ...
            T <- (A[i, j] + signp(A[i, j]) * alpha) / (a[j] - 1)
            C <- 1 / sqrt(1 + T ^ 2)
            S <- T * C
            R <- matrix(c(C, -S, S, C), 2, 2)
            A[, c(i, j)] <- A[, c(i, j)] %*% R
            A[c(i, j), ] <- crossprod(R, A[c(i, j), ])
            a[i] <- A[i, i] <- 1  # Replacing individual elements is slightly
            a[j] <- A[j, j]       # faster than a <- diag(A)
            y <- which(a < 1)
            z <- which(a > 1)
        }
        diag(A) <- 1
        A <- (A + t(A)) / 2
        ## End of adopted block 1
        ##########
        if(check) {
            svd.A <- svd(A, nu = 0)
            if(any(svd.A$d < 0)) {
        ...
        if(evectors[1] == "plain") {
            ...
            if(evectors == "MAP") {
                ##########
                ## Start of adopted block 2: largely adopted from
        ...
        ## Construct the final output from evec, evalues, and scale.
        A <- S_fromUL(evec, evalues)
        A <- (A + t(A)) / 2
        if(check) {
            svd.A <- svd(A, nu = 0)
        ...
        if(!is.null(scale.)) A <- A / sum(diag(A)) * scale.
        return(A)
    }
}
