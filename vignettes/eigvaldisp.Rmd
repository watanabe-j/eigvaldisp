...
EDI_sam <- VE(X = X)
## Same as VE(V = cov(X)) but usually faster
...
(E_V_Sigma <- Exv.VEv(V = Sigma, n = N - 1))
...
## Same as VE(V = cov2cor(Sigma))
str(VE(V = Sigma, scale. = TRUE))
...
## These are the same
all.equal(VE(X = X), VE(V = cov(X)))
all.equal(VE(X = X), VE(L = eigen(cov(X))$values))
all.equal(VE(X = X, scale. = TRUE), VE(V = cor(X)))
```
...
These functions typically take two arguments: the population covariance or correlation matrix (`V` or `R`, as appropriate), and the degrees of freedom `n`. They are (pseudo-)vectorized with respect to `n`, so that the output is a vector with the same length as `n`.

They can take a vector of population eigenvalues as the argument `L`, instead of `V` or `R`. For functions pertaining to $\V(\m{S})$ and $\Vr(\m{S})$, the results are identical (and indeed leads to slight improvement of speed when `L` is already available). On the other hand, this is usually **not** recommended for those pertaining to $\Vr(\m{R})$, because the distribution of this index is also dependent on population eigenvectors as well as population eigenvalues, except in certain restrictive conditions [see @Watanabe2021]. If the latter functions are provided with `L`, a population correlation matrix is generated with random eigenvectors, which in general leads to random fluctuation in the results (see examples below).
...
This function usually takes three arguments: `b`, the number of simulations, `V`, the population covariance matrix, and `N`, sample size.
Alternatively, parametric bootstrapping can be performed by providing a data matrix `X` (in which case `V` and `N` are not required).
...
In addition, `rmvn()` can directly take the matrix square root as the argument `cV`, which can be useful when numerous simulations are run with the same population covariance matrix.

Quick examples:
```{r}
## Full-rank covariance matrix
Sigma1 <- GenCov(evalues = c(4, 2, 1, 1), evectors = "random")
X1 <- eigvaldisp:::rmvn(10, V = Sigma1)

## Singular covariance matrix (with only 1 eigenvalue equal to zero)
Sigma2 <- GenCov(evalues = c(4, 2, 1, 0), evectors = "random")
## Not run: eigvaldisp:::rmvn(10, V = Sigma2) ## Fails
X2 <- eigvaldisp:::rmvn(10, V = Sigma2, sqrt_method = "pivot")

## Singular covariance matrix (with >1 eigenvalue equal to zero)
Sigma3 <- GenCov(evalues = c(4, 2, 0, 0), evectors = "random")
X3 <- eigvaldisp:::rmvn(10, V = Sigma3, sqrt_method = "chol_piv") ## May fail
X3 <- eigvaldisp:::rmvn(10, V = Sigma3, sqrt_method = "matsqrt")  ## Better
```
